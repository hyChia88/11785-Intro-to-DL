...
REF_RESULT_DIR = os.path.join(test_dir, 'ref_result/')
...
def test_upsampling_1d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "upsample_1d_res.npz"), 
        allow_pickle=True)
        ...
        for i in range(3):
            try:
                assert expected_res['forward_res_list'][i].shape == ep_fres_arr[i].shape, \
                    f"Shape mismatch in forward result at index {i}: Expected {expected_res['forward_res_list'][i].shape}, got {ep_fres_arr[i].shape}"
                assert expected_res['backward_res_list'][i].shape == ep_bres_arr[i].shape, \
                    f"Shape mismatch in backward result at index {i}: Expected {expected_res['backward_res_list'][i].shape}, got {ep_bres_arr[i].shape}"
                assert np.allclose(expected_res['forward_res_list'][i], ep_fres_arr[i]), \
                    f"Value mismatch in forward result at index {i}"
                assert np.allclose(expected_res['backward_res_list'][i], ep_bres_arr[i]), \
                    f"Value mismatch in backward result at index {i}"
                print(f"Passed ConvTranspose1d Test: {i+1} / 3")
            except AssertionError as ae:
                print(f"Failed ConvTranspose1d Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
        print('-' * 20)
        return True
...
def test_downsampling_1d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "downsample_1d_res.npz"), 
        allow_pickle=True)
    ...
    try:
        for i in range(3):
            try:
                ...
                print(f"Failed downsampling_1d Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
...
def test_upsampling_2d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "upsample_2d_res.npz"),
        allow_pickle=True)

    try:
        for i in range(3):
            try:
                # Check shape of forward results
                assert expected_res['forward_res_list'][i].shape == ep_fres_arr[i].shape, \
                    f"Shape mismatch in forward result at index {i}: Expected {expected_res['forward_res_list'][i].shape}, got {ep_fres_arr[i].shape}"
                
                # Check shape of backward results
                assert expected_res['backward_res_list'][i].shape == ep_bres_arr[i].shape, \
                    f"Shape mismatch in backward result at index {i}: Expected {expected_res['backward_res_list'][i].shape}, got {ep_bres_arr[i].shape}"
                
                # Check values of forward results
                assert np.allclose(expected_res['forward_res_list'][i], ep_fres_arr[i]), \
                    f"Value mismatch in forward result at index {i}"
                
                # Check values of backward results
                assert np.allclose(expected_res['backward_res_list'][i], ep_bres_arr[i]), \
                    f"Value mismatch in backward result at index {i}"
                
                print(f"Passed upsampling_2d Test: {i+1} / 3")
            except AssertionError as ae:
                print(f"Failed upsampling_2d Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
...
def test_downsampling_2d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "downsample_2d_res.npz"),
        allow_pickle=True)
    ...
    try:
        for i in range(3):
            try:
                ...
                print(f"Failed downsampling_2d Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
...
def test_cnn_correctness_conv1d_stride1_once(idx):
    ...
    ##########################################################################
    ##########    Initialize the CNN layer and copy parameters to a PyTorch CN
    ##########################################################################
    ...
    if not assertions(test_y, torch_y, 'type', 'y'):
        return scores_dict
    if not assertions(test_y, torch_y, 'shape', 'y'):
        return scores_dict
    if not assertions(test_y, torch_y, 'closeness', 'y'):
        return scores_dict
    scores_dict[0] = 1
    ...
    ##########################################################################
    ################   Check your dLdA, dLdW and dLdb with PyTorch build-in fu
    ##########################################################################
...
def test_cnn_correctness_conv1d_once(idx):
    ...
    ##########################################################################
    ################   Check your dx, dLdW and dLdb with PyTorch build-in func
    ##########################################################################
    dx1 = x1.grad.detach().numpy()
    delta_res_norm = abs(dx - dx1).max()
...
def conv2d_correctness_stride1():
    ...
    #################################    Create Models   #####################
    ##########################################################################
    def random_normal_weight_init_fn(
            out_channels, in_channels, kernel_width, kernel_height):
        return np.random.normal(
            0, 1.0, (out_channels, in_channels, kernel_width, kernel_height))

    test_model = Conv2d_stride1(
        in_c,
        out_c,
        kernel,
        random_normal_weight_init_fn,
    ...
    if not assertions(test_dx, torch_dx, 'type', 'dLdA'):
        return scores_dict
    if not assertions(test_dx, torch_dx, 'shape', 'dLdA'):
        return scores_dict
    if not assertions(test_dx, torch_dx, 'closeness', 'dLdA'):
        return scores_dict
    print('Conv2d_stride1 Backward dLdA: PASS')
    scores_dict[1] = 1

    if not assertions(test_dW, torch_dW, 'type', 'dLdW'):
        return scores_dict
    if not assertions(test_dW, torch_dW, 'shape', 'dLdW'):
        return scores_dict
    if not assertions(test_dW, torch_dW, 'closeness', 'dLdW'):
        return scores_dict
    print('Conv2d_stride1 Backward dLdW: PASS')
    scores_dict[2] = 1

    if not assertions(test_db, torch_db, 'type', 'dLdb'):
        return scores_dict
    if not assertions(test_db, torch_db, 'shape', 'dLdb'):
        return scores_dict
    if not assertions(test_db, torch_db, 'closeness', 'dLdb'):
        return scores_dict
    ...
    for i in range(n):
        ...
        if a != 1:
            if __name__ == '__main__':
                print(
                    'Failed Conv2d_stride1 Forward Test: %d / %d' %
                    (i + 1, n))
            return False
        else:
            if __name__ == '__main__':
                print(
                    'Passed Conv2d_stride1 Forward Test: %d / %d' %
                    (i + 1, n))
        if b != 1 or c != 1 or d != 1:
            if __name__ == '__main__':
                print(
                    'Failed Conv2d_stride1 Backward Test: %d / %d' %
                    (i + 1, n))
            return False
        else:
            if __name__ == '__main__':
                print(
                    'Passed Conv2d_stride1 Backward Test: %d / %d' %
                    (i + 1, n))
    print('-' * 20)
    return True
...
def weight_init_fn_2d(out_channels, in_channels, kernel_width1, kernel_width2):
    np.random.seed(11785)
    return np.random.normal(
        0, 1.0, (out_channels, in_channels, kernel_width1, kernel_width2))
...
def test_convTranspose_1d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "convTranspose_1d_res.npz"),
        allow_pickle=True)
        ...
        for i in range(3):
            try:
                ...
                assert expected_res['conv_forward_res_list'][i].shape == ep_fres_arr[i].shape, \
                    f"Shape mismatch in conv_forward result at index {i}: Expected {expected_res['conv_forward_res_list'][i].shape}, got {ep_fres_arr[i].shape}"
                
                # Check shape of backward results
                assert expected_res['conv_backward_res_list'][i].shape == ep_bres_arr[i].shape, \
                    f"Shape mismatch in conv_backward result at index {i}: Expected {expected_res['conv_backward_res_list'][i].shape}, got {ep_bres_arr[i].shape}"
                
                # Check values of forward results
                assert np.allclose(expected_res['conv_forward_res_list'][i], ep_fres_arr[i]), \
                    f"Value mismatch in conv_forward result at index {i}"
                
                # Check values of backward results
                assert np.allclose(expected_res['conv_backward_res_list'][i], ep_bres_arr[i]), \
                    f"Value mismatch in conv_backward result at index {i}"
                ...
                print(f"Details: {ae}")
                raise
        print("ConvTranspose1d: PASS")
        print('-' * 20)
        return True
    except Exception as e:
        print("ConvTranspose1d: FAIL")
        print('-' * 20)
        traceback.print_exc()
        return False
...
def test_convTranspose_2d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "convTranspose_2d_res.npz"),
        allow_pickle=True)
    ...
    try:
        for i in range(3):
            try:
                ...
                print(f"Failed ConvTranspose2d Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
...
def test_MaxPool2d_stride1_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "maxpool2d_stride1_res.npz"),
        allow_pickle=True)
        ...
        for i in range(3):
            try:
                ...
                assert expected_res['maxpool_forward_res_list'][i].shape == ep_fres_arr[i].shape, \
                    f"Shape mismatch in maxpool_forward result at index {i}: Expected {expected_res['maxpool_forward_res_list'][i].shape}, got {ep_fres_arr[i].shape}"
                
                # Check shape of backward results
                assert expected_res['maxpool_backward_res_list'][i].shape == ep_bres_arr[i].shape, \
                    f"Shape mismatch in maxpool_backward result at index {i}: Expected {expected_res['maxpool_backward_res_list'][i].shape}, got {ep_bres_arr[i].shape}"
                
                # Check values of forward results
                assert np.allclose(expected_res['maxpool_forward_res_list'][i], ep_fres_arr[i]), \
                    f"Value mismatch in maxpool_forward result at index {i}"
                
                # Check values of backward results
                assert np.allclose(expected_res['maxpool_backward_res_list'][i], ep_bres_arr[i]), \
                    f"Value mismatch in maxpool_backward result at index {i}"
                
                print(f"Passed Maxpool2d_stride1 Test: {i+1} / 3")
            except AssertionError as ae:
                print(f"Failed Maxpool2d_stride1 Test at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
        print("Maxpool2d_stride1: PASS")
        print('-' * 20)
        return True
    except Exception as e:
        print("Maxpool2d_stride1: FAIL")
        print('-' * 20)
        traceback.print_exc()
        return False
...
def test_MaxPool2d_correctness():
    ...
    expected_res = np.load(
        os.path.join(REF_RESULT_DIR, "maxpool2d_res.npz"),
        allow_pickle=True)
    ...
    try:
        for i in range(3):
            try:
                # Check shape of forward results
                assert expected_res['maxpool_forward_res_list'][i].shape == ep_fres_arr[i].shape, \
                    f"Shape mismatch in forward result at index {i}: Expected {expected_res['maxpool_forward_res_list'][i].shape}, got {ep_fres_arr[i].shape}"

                # Check shape of backward results
                assert expected_res['maxpool_backward_res_list'][i].shape == ep_bres_arr[i].shape, \
                    f"Shape mismatch in backward result at index {i}: Expected {expected_res['maxpool_backward_res_list'][i].shape}, got {ep_bres_arr[i].shape}"
                ...
                print(f"Test failed at index {i+1} / 3")
                print(f"Details: {ae}")
                raise
        print("Maxpool2d Test: PASS")
        print('-' * 20)
        return True
    except Exception as e:
        print("Maxpool2d Test: FAIL")
        print('-' * 20)
        traceback.print_exc()
        return False
...
def mean_pool_correctness_stride1():
    ...
    if not assertions(test_xp, torch_xp, 'type', 'dLdA'):
        return scores_dict
    if not assertions(test_xp, torch_xp, 'shape', 'dLdA'):
        return scores_dict
    if not assertions(test_xp, torch_xp, 'closeness', 'dLdA'):
        return scores_dict
    scores_dict[1] = 1
...
def test_mean_pool_stride1():
    ...
    for i in range(n):
        ...
        if a != 1:
            if __name__ == '__main__':
                print(
                    'Failed MeanPool2d_stride1 Forward Test: %d / %d' %
                    (i + 1, n))
            return False
        elif b != 1:
            if __name__ == '__main__':
                print(
                    'Failed MeanPool2d_stride1 Backward Test: %d / %d' %
                    (i + 1, n))
            return False
        else:
            if __name__ == '__main__':
...
def test_simple_scanning_mlp():
    data = np.loadtxt(os.path.join(DATA_DIR, "data.asc")).T.reshape(1, 24, -1)
    
    cnn = cnn_solution.CNN_SimpleScanningMLP()
    weights = np.load(
        os.path.join(WEIGHTS_DIR, 'mlp_weights_part_b.npy'), 
        allow_pickle=True)
    cnn.init_weights(weights)

    expected_result = np.load(
        os.path.join(REF_RESULT_DIR, 'res_b.npy'), 
        allow_pickle=True)
    result = cnn.forward(data) # here

    try:
        # Check if result type matches the expected type
        assert isinstance(result, type(expected_result)), \
            f"Type mismatch: Expected type {type(expected_result)}, got {type(result)}"

        # Check if result shape matches the expected shape
        assert result.shape == expected_result.shape, \
            f"Shape mismatch: Expected shape {expected_result.shape}, got {result.shape}"

        # Check if the result values are close to the expected values
        assert np.allclose(result, expected_result), \
            "Value mismatch: Result does not match expected values"
...
def test_distributed_scanning_mlp():
    ...
    weights = np.load(os.path.join(WEIGHTS_DIR, 'mlp_weights_part_c.npy'), allow_pickle=True)
    cnn.init_weights(weights)

    expected_result = np.load(os.path.join(REF_RESULT_DIR, 'res_c.npy'), allow_pickle=True)
...
def get_cnn_model():
    ...
    model = cnn.CNN(input_width, input_channels, out_channels, kernel_sizes, strides, num_linear_neurons,
                    activations, conv_weight_init_fn, bias_init_fn, linear_weight_init_fn,
                    criterion, lr)

    model.linear_layer.W = linear_weight_init_fn(
        model.linear_layer.W.shape[0],
        model.linear_layer.W.shape[1])
    model.linear_layer.b = bias_init_fn(
        model.linear_layer.b.shape[0]).reshape(-1, 1)
    return model
...
def cnn_model_correctness(idx):
    ...
    ref_model.fc.weight = nn.Parameter(
        torch.tensor(submit_model.linear_layer.W))
...
if DEBUG_AND_GRADE_DOWNSAMPLING_1D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_UPSAMPLING_1D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_DOWNSAMPLING_2D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_UPSAMPLING_2D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONV1D_STRIDE1_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONV1D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONV2D_STRIDE1_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONV2D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONVTRANSPOSE1D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONVTRANSPOSE2D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_MAXPOOL2D_STRIDE1_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_MAXPOOL2D_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_MEANPOOL_STRIDE1_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_MEANPOOL_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_SIMPLE_SCAN_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_DISTRIBUTED_SCAN_flag:
    tests.append(
        {
...
if DEBUG_AND_GRADE_CONV1D_MODEL_flag:
    tests.append(
        {
