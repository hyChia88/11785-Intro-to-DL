...
class Tanh(Identity):
    ...
    def backward(self, dLdA):
        ...
        dLdZ = tanh.backward(dLdA)
             = 1 - tanh^2(dLdA) # A = tanh(Z) in forward pass
             = dL/dA * (1 - A^2)
        """
...
class ReLU(Identity):
    ...
    def backward(self, dLdA):
        ...
        dLdZ = relu.backward(dLdA)
             = (? , A > 0; ? , A ≤ 0)
        """
        # Element-wise check, Gradient passes through where A > 0, Zero gradient where A ≤ 0
        dLdZ = dLdA * (self.A > 0)
        return dLdZ
...
class GELU(Identity):
    ...
    def forward(self, Z):
        self.Z = Z
        # scalar use *, Element-wise multiplication
        self.A = ((1/2)*Z) * (1 + sp.erf(Z/np.sqrt(2)))
        return self.A
    
    def backward(self, dLdA):
        Z = self.Z
        dAdZ = 0.5*(1 + sp.erf(Z/np.sqrt(2))) + (Z/np.sqrt(2*np.pi)) * np.exp(-Z**2/2)
        dLdZ = dLdA * dAdZ
        return dLdZ
...
class Softmax(Identity):
    ...
    def forward(self, Z):
        ...
        Note: How can we handle large overflow values? Hint: Check numerical stability.
            Numerical Stability, ensures that our computation remains well-defined by subtracting the maximum value within each row before exponentiation (zm). By performing this row-wise subtraction, we keep all exponentiated values within a safer range, preventing them from becoming too large and causing arithmetic overflow.
        """
        Zm = Z - np.max(Z, axis=1, keepdims=True)
        sum_Zm = np.sum(np.exp(Zm), axis=1, keepdims=True)
        self.A = np.exp(Zm) / sum_Zm
        return self.A

    def backward(self, dLdA):
        # Calculate the batch size and number of features
        N = dLdA.shape[0] # batch size
        C = dLdA.shape[1] # number of features
        ...
        for i in range(N):
            ...
            # Hint: Jacobian matrix for softmax is a _×_ matrix, but what is _ here? _ is C
            J = np.zeros((C,C))
            ...
            for m in range(C):
                for n in range(C):
                    if m == n:
                        # a_m = self.A, where am refers to the m-th element of the vector A.
